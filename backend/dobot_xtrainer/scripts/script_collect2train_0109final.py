...
def threshold_binarize_gripper(gripper_vals, thresh=0.5):
    """
    gripper_vals: (T,) 连续值，范围 [0, 1]
    return: (T,) 二值夹爪信号 {-1, 1}
    """
    out = np.where(gripper_vals < thresh, -1.0, 1.0)
    return out
...
def main(root_dir, dataset_name, date_collect, idx):
    ...
    # ====== 新增：夹爪二值化（必须在 action 之前） ======
    left_gripper_idx = 6
    right_gripper_idx = 13

    action[:, left_gripper_idx] = threshold_binarize_gripper(
        action[:, left_gripper_idx]
    )
    action[:, right_gripper_idx] = threshold_binarize_gripper(
        action[:, right_gripper_idx]
    )
