...
# Compiled at: 2024-05-07 16:59:12
            ...
            with h5py.File(dataset_path, "r") as root:
                try:
                    ...
                    if compressed:
                        ...
                        action = action[start_ts[:None]]
                        action_len = episode_len - start_ts
                    else:
                        action = action[max(0, start_ts - 1)[:None]]
                        action_len = episode_len - max(0, start_ts - 1)
            padded_action = np.zeros((self.max_episode_len, original_action_shape[1]), dtype=(np.float32))
            padded_action[None[:action_len]] = action
            is_pad = np.zeros(self.max_episode_len)
            is_pad[action_len[:None]] = 1
            padded_action = padded_action[None[:self.chunk_size]]
            is_pad = is_pad[None[:self.chunk_size]]
            all_cam_images = []
            for cam_name in self.camera_names:
...
def load_data(dataset_dir_l, name_filter, camera_names, batch_size_train, batch_size_val, chunk_size, skip_mirrored_data=False, load_pretrain=False, policy_class=None, stats_dir_l=None, sample_weights=None, train_ratio=0.99):
    ...
    train_episode_ids_0 = shuffled_episode_ids_0[None[:int(train_ratio * num_episodes_0)]]
    val_episode_ids_0 = shuffled_episode_ids_0[int(train_ratio * num_episodes_0)[:None]]
    train_episode_ids_l = [train_episode_ids_0] + [np.arange(num_episodes) + num_episodes_cumsum[idx] for idx, num_episodes in enumerate(num_episodes_l[1[:None]])]
...
def smooth_base_action(base_action):
    return np.stack([np.convolve((base_action[(None[:None], i)]), (np.ones(5) / 5), mode="same") for i in range(base_action.shape[1])],
...
def sample_box_pose():
    ...
    cube_position = np.random.uniform(ranges[(None[:None], 0)], ranges[(None[:None], 1)])
...
def sample_insertion_pose():
    ...
    z_range = [0.05, 0.05]
    ranges = np.vstack([x_range, y_range, z_range])
    peg_position = np.random.uniform(ranges[(None[:None], 0)], ranges[(None[:None], 1)])
    ...
    z_range = [0.05, 0.05]
    ranges = np.vstack([x_range, y_range, z_range])
    socket_position = np.random.uniform(ranges[(None[:None], 0)], ranges[(None[:None], 1)])
