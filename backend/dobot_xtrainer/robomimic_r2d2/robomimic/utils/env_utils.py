...
def get_env_class(env_meta=None, env_type=None, env=None):
    ...
    if env_type == EB.EnvType.ROBOSUITE_TYPE:
        from backend.dobot_xtrainer.robomimic_r2d2.robomimic.envs.env_robosuite import EnvRobosuite
        return EnvRobosuite
    elif env_type == EB.EnvType.GYM_TYPE:
        from backend.dobot_xtrainer.robomimic_r2d2.robomimic.envs.env_gym import EnvGym
        return EnvGym
    elif env_type == EB.EnvType.IG_MOMART_TYPE:
        from backend.dobot_xtrainer.robomimic_r2d2.robomimic.envs.env_ig_momart import EnvGibsonMOMART
...
def wrap_env_from_config(env, config):
    ...
    if config.train.frame_stack > 1:
        from backend.dobot_xtrainer.robomimic_r2d2.robomimic.envs.wrappers import FrameStackWrapper
