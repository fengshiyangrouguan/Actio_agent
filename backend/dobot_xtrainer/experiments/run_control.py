...
# 0: lock, 1: unlock     短按黄色按键
# 0: stop servo, 1: servo   长按黄色按键
# 0: stop recording, 1: recording     短按绿色按键
"""
按键监控与状态标志:
what_to_do是一个 2x3 的数组,2是左右两个机械臂，3是三个操作
当检测到使能标志变化（what_to_do[i, j] != 0）时，就执行相应任务
"""
what_to_do = np.array(([0, 0, 0], [0, 0, 0]))
dt_time = np.array([20240507161455])
using_sensor_protection = False    #跌倒保护标志位
is_falling = np.array([0])     #全局跌倒标志
...
def run_thread_cam(rs_cam, which_cam):
    ...
    while 1:
        # 根据不同相机位置进行不同处理
        if which_cam == 1 or which_cam == 2 or which_cam == 3 or which_cam == 0:
            # 左侧、右侧、底部相机：直接读取并转换颜色空间
            image_cam, _ = rs_cam.read()  # 从相机读取一帧图像
            image_cam = image_cam[:, :, ::-1]  # 将RGB转换为BGR（OpenCV格式）
        
        # elif which_cam == 0:
        #     # 顶部相机：特殊裁剪处理
        #     image_top_src, _ = rs_cam.read()  # 读取原始图像
        #     # 裁剪图像：取行150-420，列220-480区域
...
x_range：X轴安全范围 [x_min, x_max]
y_range：Y轴安全范围 [y_min, y_max]
z_min：Z轴最低安全阈值（机械臂底座为参考
"""
...
def check_pose_protection(positions, vel, what_to_do):
    ...
    # 工作空间安全边界定义
    x_range_left = (-450, 450)  # 左臂X轴范围：-450mm 到 300mm
    x_range_right = (-450, 450)  # 右臂X轴范围：-250mm 到 450mm
    y_range = (-750, 750)  # Y轴公共范围：-750mm 到 -160mm
    z_range_left = 500  # 左臂最低安全高度：40mm
    z_range_right = 500  # 右臂最低安全高度：40mm

    if what_to_do[0, 1]:  # The left hand is in sync
        # Z direction speed limit -1 m/s
        # 速度安全检测: 检查机械臂在Z轴方向的运动速度，防止过快下降造成撞击
        ...
        positions_to_check = ['left_left', 'left_right']     #左臂的左右两个夹爪
        x_ranges = [x_range_left, x_range_left]      #[(-450, 300), (-450, 300)]  分别代表左臂的左右两个夹爪
...
def check_joint_safety(action):
    """
        检查机械臂关节角度是否处于安全位置范围（是角度）

        参数:
            action: list/numpy.array
                包含机械臂所有关节角度值的数组，长度为14（左右机械臂各7个关节）
                索引说明:
                - action[2]: 左机械臂J3关节角度
                - action[9]: 右机械臂J3关节角度
...
def main(args):
    ...
    while 1:
        ...
        assert thread_cam_bottom.is_alive(), "Error: please check the bottom camera!"
        ...
        # 1. 切换机械臂的锁定/解锁状态
        for i in range(2):
            if dev_what_to_do[i, 0] != 0:     # 检查第i个机械臂的锁定状态是否发生变化（dev_what_to_do是状态变化量）
                agent.set_torque(i, not what_to_do[i, 0])    # 设置机械臂扭矩：参数1为机械臂索引，参数2取反当前状态（True启用扭矩/解锁，False禁用扭矩/锁定）

        # button A: long press event. servo or not
        # 2. 切换机械臂的伺服模式
        if dev_what_to_do[0, 1] == 1 or dev_what_to_do[1, 1] == 1:    #检测左右机械臂（索引0和1）的伺服使能状态是否发生了变化
            ...
            last_action = dynamic_approach(env, agent, flag_in)     #规划并执行一条从机械臂当前位置平滑运动到智能体（Agent）起始工作点的轨迹。这避免了伺服模式开始时因位置突变造成的冲击
            for i in range(2):
                if what_to_do[i, 0]:
                    if what_to_do[i, 1]:
        ...
        if (what_to_do[0, 1] or what_to_do[1, 1]) and start_servo:
            ...
            # 3. 控制视频录制
            if dev_what_to_do[0, 2] == 1:    # 检测到录制开始信号
        ...
        if args.show_img:
            # 将三视角图像拼接为全景画面
            show_canvas[:, :640] = np.asarray(img_list[0], dtype="uint8")  # 顶视角图像置于左侧
            show_canvas[:, 640:640 * 2] = np.asarray(img_list[1], dtype="uint8")  # 左视角图像置于中部
            show_canvas[:, 640 * 2:640 * 3] = np.asarray(img_list[2], dtype="uint8")  # 右视角图像置于右侧
            show_canvas[:, 640 * 3:640 * 4] = np.asarray(img_list[3], dtype="uint8")  # 底部视角图像置于右侧
            cv2.imshow("0", show_canvas)  # 显示拼接后的图像窗口
            cv2.waitKey(1)  # 短暂等待（1ms）以刷新显示
        # 性能监控：计算单次循环耗时
        toc = time.time()
        total_time = toc-tic
